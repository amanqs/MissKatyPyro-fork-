...
async def payment(client: Client, message: Message):
    ...
    if message.chat.type.value != "private":
        msg = await message.reply(payment_guide, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Payment Web", url=checkout_url)]]), quote=True)
    else:
        msg = await message.reply(payment_guide, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Payment Web", web_app=WebAppInfo(url=checkout_url))]]), quote=True)
